...
// Use your mouse to move the camera around! Press the Left Mouse Button on the
...
// #define NormalizedMouse (iMouse.xy / iResolution.xy) // normalize mouse
// coords

// Calculates wave value and its derivative,
// for the wave direction, position in space, wave frequency and time
float2 wavedx(float2 position, float2 direction, float frequency,
              float timeshift) {
...
float getwaves(float2 position, int iterations) {
    float iter =
        0.0; // this will help generating well distributed wave directions
    float frequency =
        1.3; // frequency of the wave, this will change every iteration
    float timeMultiplier =
        1.0; // time multiplier for the wave, this will change every iteration
    float weight = 0.5; // weight in final sum for the wave, this will change
                        // every iteration
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        // calculate wave data
        float2 res =
            wavedx(position, p, frequency, globalTime * timeMultiplier);

        // shift position around according to wave drag and derivative of the
        // wave
...
float raymarchwater(float3 camera, float3 start, float3 end, float depth) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        // if the waves height almost nearly matches the ray height, assume its
        // a hit and return the hit distance
...
// Calculate normal at point by calculating the height at the pos and 2
// additional points very close to pos
float3 normal(float2 pos, float e, float depth) {
    ...
    return normalize(cross(
        a - float3(pos.x - e,
                   getwaves(pos.xy - ex.xy, ITERATIONS_NORMAL) * depth, pos.y),
        a - float3(pos.x, getwaves(pos.xy + ex.yx, ITERATIONS_NORMAL) * depth,
                   pos.y + e)));
}

// Ray-Plane intersection checker
float intersectPlane(float3 origin, float3 direction, float3 pos,
                     float3 normal) {
    return clamp(dot(pos - origin, normal) / dot(direction, normal), -1.0,
                 9991999.0);
}
...
float3 extra_cheap_atmosphere(float3 raydir, float3 sundir) {
    ...
    float3 suncolor = lerp(
        float3(1, 1, 1),
        max(float3(1, 1, 1), float3(1, 1, 1) - float3(5.5, 13.0, 22.4) / 22.4),
        special_trick2);
...
float4 main(PixelShaderInput input) : SV_Target0 {
    ...
    float highPlaneHit =
        intersectPlane(origin, ray, waterPlaneHigh, float3(0.0, 1.0, 0.0));
    float lowPlaneHit =
        intersectPlane(origin, ray, waterPlaneLow, float3(0.0, 1.0, 0.0));
    ...
    float3 scattering = float3(0.0293, 0.0698, 0.1717) *
